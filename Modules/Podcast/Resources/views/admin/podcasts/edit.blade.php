...
@section('content-header')
    <h1>
        {{ trans('podcast::podcasts.title.edit podcast') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.podcast.podcast.index') }}">{{ trans('podcast::podcasts.title.podcasts') }}</a></li>
        <li class="active">{{ trans('podcast::podcasts.title.edit podcast') }}</li>
    </ol>
@stop
...
@section('content')
    {!! Form::open(['route' => ['admin.podcast.podcast.update', $podcast->id], 'method' => 'put']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('podcast::admin.podcasts.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            $(document).keypressAction({
                actions: [
                    { key: 'b', route: "<?= route('admin.podcast.podcast.index') ?>" }
                ]
            });
        });
    </script>
